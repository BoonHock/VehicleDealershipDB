CREATE TABLE [hr].[salesperson] (
    ...
    CONSTRAINT [CK_salesperson] CHECK ([person] IS NULL AND [organisation] IS NOT NULL OR [person] IS NOT NULL AND [organisation] IS NULL AND [organisation_branch] IS NULL),
    CONSTRAINT [FK_salesman_organisation] FOREIGN KEY ([organisation]) REFERENCES [hr].[organisation] ([organisation]),
